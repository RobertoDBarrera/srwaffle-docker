...
// 1. Add 'themes' to adminViews
if (!code.includes("themes: document.getElementById('admin-view-themes')")) {
  code = code.replace(
...
// 2. Add 'loadThemesGallery' to switchAdminView
if (!code.includes("if (viewName === 'themes') loadThemesGallery();")) {
  code = code.replace(
...
if (!code.includes("const loadThemesGallery = async () => {")) {
...
`;
  ...
  // Find where to append. Append before the last line `})();`
  const lastIndex = code.lastIndexOf('})();');
  if (lastIndex !== -1) {
    code = code.substring(0, lastIndex) + implementation + '\n' + code.substring(lastIndex);
  }
}

fs.writeFileSync('admin/app.js', code);
console.log('admin/app.js updated successfully');
